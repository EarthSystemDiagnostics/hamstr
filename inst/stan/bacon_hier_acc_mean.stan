...
// 29.09.2017 Andrew Dolman
// Make Bacon Hierarchical
data {
  int<lower=0> N;
  int<lower=0> K;
  int<lower=0> nu;
  ...
  // hyperparameters
  real<lower = 0> acc_mean_mean;
  real<lower = 0> acc_mean_shape;
...
transformed data{
  ...
  real<lower=0> acc_mean_alpha;
  real<lower=0> acc_mean_beta;
  ...
  acc_mean_alpha = acc_mean_shape;
  acc_mean_beta = acc_mean_shape / acc_mean_mean;
...
model {
  acc_mean ~ gamma(acc_mean_alpha, acc_mean_beta);
