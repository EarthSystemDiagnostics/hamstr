...
print(twoFit$fit, par = c("obs_err_1"))

print(twoFit$fit, par = c("infl_mean"))

print(twoFit$fit, par = c("infl_shape"))
print(twoFit$fit, par = c("infl_alpha"))
print(twoFit$fit, par = c("infl_beta"))


alpha <- s1 %>% 
  filter(par == "infl_alpha") %>% pull(mean)

beta <- s1 %>% 
  filter(par == "infl_beta") %>% pull(mean)


x <- seq(-0.1, 10, length.out = 1000)
d <- dgamma(x, shape = alpha, rate = beta)
plot(x, d, type = "l", xlim = c(0, 11))


print(twoFit$fit, par = c("record_acc_shape"))
