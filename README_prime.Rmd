...
```{r}
hamstr_fit_1p <- hamstr_prime(depth = MSB2K_cal$depth,
...
```{r, dev = "png"}
plot(hamstr_fit_1p, summarise = FALSE, plot_diagnostics = FALSE)
```
...
```{r}
plot(hamstr_fit_1p, type = "acc_mean_prior_post")
```
...
The key structural difference between **Bacon** and **hamstr** models is that with **hamstr** the sediment core is modelled at multiple resolutions simultaneously, with a hierarchical structure. This removes the need to trade-off smoothness and flexibility. The accumulation rates estimated for (parent) sections in low resolution levels act as priors for (child) sections in subsequent higher resolution levels. The hierarchical structure allows the model to adapt to low-frequency changes in the accumulation rate, that is changes between "regimes" of high or low accumulation that persist for long periods. 

As of **hamstr** version 0.8, subsequent hierarchical levels are offset from each other so that child sections can have priors from 2 parents, this smooths out the edge effects of the the breakpoints in the low resolution layers.

The parameter `K_fine` sets the number of sections in the final, highest resolution, age-depth model. `K_factor` controls how the number of sections changes between levels. A value of e.g. 2 for `K_factor` means that the number of sections approximately halves/doubles between hierarchical levels. 

By default `K_factor` is chosen so that the number of hierarchical levels increases slowly with the value of `K_fine`. For example, with K_fine = 100, K_factor = 4 and the number of hierarchical levels is 5. With K_fine = 1000, K_factor = 5 and the number of hierarchical levels is 6. The number of hierarchical levels is approximately equal to the ratio between the number of sections in adjacent levels.
...
```{r}
age.mods.interp <- predict(hamstr_fit_1p, depth = seq(0, 100, by = 1))
```
...
```{r}
plot(hamstr_fit_1p, type = "acc_rates")
```

```{r}
summary(hamstr_fit_1p, type = "acc_rates") 
```
...
```{r}
plot(hamstr_fit_1p, type = "hier_acc")
```
...
```{r}
rstan::check_divergences(hamstr_fit_1p$fit)

rstan::stan_rhat(hamstr_fit_1p$fit)
```

The first `alpha` parameter is the overall mean accumulation rate.

```{r}
rstan::traceplot(hamstr_fit_1p$fit, par = c("alpha[1]"),
                 inc_warmup = TRUE)
```
