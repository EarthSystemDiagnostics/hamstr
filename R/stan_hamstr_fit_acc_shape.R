...
#'   \link[MASS]{rlm}.
#' @param acc_shape_mean description
#' @param acc_shape_shape description
...
#'
#' }
hamstr_fit_acc_shape <- function(depth, obs_age, obs_err,
  ...
  stan_dat <- make_stan_dat_hamstr_fit_acc_shape()
  ...
  inits <- replicate(used_sampler_args$chains,
                     list(get_inits_hamstr_fit_acc_shape(stan_dat)))
  
  
  
  args <- list(object = stanmodels$hamstr_fit_acc_shape, data = stan_dat,
               init = inits)
