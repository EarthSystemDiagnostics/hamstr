...
```{r}
adam_fit_5 <- adam(depth = MSB2K_cal$depth,
                   obs_age = MSB2K_cal$age.14C.cal,
                   obs_err = MSB2K_cal$age.14C.cal.se,
                   K = c(10, 10),
                   mem_mean = 0.5, mem_strength = 2,
...
plot_adam(adam_fit_5)

print(adam_fit_5$fit, pars = c("infl", "infl_mean", "infl_shape", "infl_shape_1"))

traceplot(adam_fit_5$fit, pars = c("infl_mean", "infl_shape"), inc_warmup = T)

library(rstan)
plot_infl_prior_posterior(adam_fit_5)
```
